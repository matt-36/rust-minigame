...
fn main() -> Result<(), String> {
    ...
    #[cfg(feature = "unsafe_textures")]
    {
        ...
        game.add_collider(wall);
        let sdl_context = Rc::new(RefCell::new(sdl_context));
        let canvas = Rc::new(RefCell::new(canvas));
        let game = Rc::new(RefCell::new(game));
        let _ttf_context = Rc::new(RefCell::new(_ttf_context));
        // let texture_creator = Rc::new(RefCell::new(texture_creator));
        emscripten::set_main_loop_callback(
            gameloop::game_loop(
                Rc::clone(&sdl_context), 
                Rc::clone(&canvas), 
                Rc::clone(&game),
                Rc::clone(&_ttf_context),
            )
        );
        let game: Result<(), ()> = Ok(());
        Ok(())
